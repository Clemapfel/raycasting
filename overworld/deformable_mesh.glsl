...
vec4 effect(vec4 color, sampler2D img, vec2 _, vec2 frag_position) {
    ...
    float hue = gradient_noise(vec3(rotate(uv * 2, elapsed / 300 * 2 * PI) * 2, elapsed / 7));
    return vec4(mix(vec3(0), vec3(hue), gaussian(1 - opacity, 0.7)), 1);
}
