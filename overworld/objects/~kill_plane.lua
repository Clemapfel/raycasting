...
local _shader = rt.Shader("overworld/objects/kill_plane.glsl")
...
function ow.KillPlane:draw()
    ...
    _shader:bind()
    _shader:send("elapsed", rt.SceneManager:get_elapsed() + meta.hash(self))
    _shader:send("player_position", player_position)
    _shader:send("screen_to_world_transform", transform)
    _shader:send("center", { offset_x, offset_y })
