...
vec4 effect(vec4 vertex_color, sampler2D img, vec2 texture_coords, vec2 frag_position) {
    ...
    float dip = (gradient_noise(vec3(texture_coords.xx * 7.0, 0.5 * elapsed)) + 1.0) / 2.0;
    float y = gaussian(distance(texture_coords.y * dip, 0.0), 4.0);
    float x = gaussian(distance(texture_coords.x, 0.5), 2.0);

    float flicker = (gradient_noise(vec3(texture_coords.yy, 0.5 * elapsed)) + 1.0) / 2.0;
    x *= flicker;
    ...
    return mix(
        ...
        vec4(1.0),
        x * y * (pulse + 0.4))
    * vec4(1.0, 1.0, 1.0, 2.0 * max(pulse, 0.5) * x * y)
    * vec4(vec3(brightness_scale), 1.0);
}
