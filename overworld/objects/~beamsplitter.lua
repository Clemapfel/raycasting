...
function ow.BeamSplitter:draw()
    ...
    love.graphics.line(
        self._split_origin_x,
        self._split_origin_y,
        self._split_origin_x + self._split_dx * 10,
        self._split_origin_y + self._split_dy * 10
    )
end
...
function ow.BeamSplitter:split_ray(contact_x, contact_y, dx, dy, normal_x, normal_y)
    ...
    local top_left_x, top_left_y = x, y
    local top_right_x, top_right_y = x + w, y
    local bottom_right_x, bottom_right_y = x + w, y + h
    local bottom_left_x, bottom_left_y = x, y + h
    ...
    -- get split beam
    local line_x1, line_y1 = contact_x, contact_y
    local line_x2, line_y2 = intersection_x, intersection_y

    self._split_origin_x = (line_x1 + line_x2) / 2
    self._split_origin_y = (line_y1 + line_y2) / 2

    local new_dx = line_x2 - line_x1
    local new_dy = line_y2 - line_y1

    self._split_dx = -1 * new_dy
    self._split_dy = new_dx
