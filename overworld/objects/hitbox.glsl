...
vec4 effect(vec4 color, Image img, vec2 texture_coordinates, vec2 frag_position) {
    ...
    float dist = gaussian(distance(norm_player, uv), 12 - mix(0, 3, player_flow));
    vec3 highlight = dist * player_color.rgb + smoothstep(1 - 0.5, 1.5, dist);

    // Combine quantized background with highlight
    return color + vec4(highlight * 0.7, 1);
}
