...
vec4 effect(vec4 color, sampler2D tex, vec2 texture_coords, vec2 screen_coords) {
    const float noise_scale = 1. / 20;
    const float time_scale = 1. / 4;

    vec2 world_pos = to_world_position(screen_coords) * 1.2;
    float noise = gradient_noise(vec3(world_pos * noise_scale, elapsed * time_scale));
    return vec4(color.rgb, 1) * vec4(mix(0.7, 1, (noise + 1) / 2)); //lch_to_rgb(vec3(0.8, 1, noise)), 1);
}
