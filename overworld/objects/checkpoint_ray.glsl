...
vec4 effect(vec4 color, sampler2D image, vec2 texture_coords, vec2 vertex_position) {
    ...
    vec2 normalization = vec2(1.0, (size.y / size.x));
    float strike = gaussian(distance(texture_coords * normalization, vec2(0.5, 1.0) * normalization), 1.5);

    float distortion_strength = (1.0 - distance(texture_coords.x, 0.5) * 2.0) ;
    vec2 distortion_scale = vec2(1.8, 8.0);
    float ground_weight = 1.0 - gaussian(1.0 - texture_coords.y, 12.0); // so ray touches bottom at center

    vec2 norm = size / max(size.x, size.y);
    const float ball_eps = 0.5;
    float ball = gaussian(distance(texture_coords * norm, vec2(0.5, fraction - 26.0 / size.y) * norm), 2.0);

    float ball_attenuation = texture_coords.y > fraction ? gaussian(texture_coords.y - fraction, 10.0) : 1.0;
    ball *= ball_attenuation * (texture_coords.y > 0.0 ? 1.0 : 0.0);
    ...
    float fade = 1.0 - gaussian(texture_coords.y - fraction, 15.0); // multiply so fade out is below ground
    if (texture_coords.y > fraction) fade = 0.0;

    texture_coords.x += distortion.x * ground_weight;

    float ray_width = 4.0;
    float ray_outer = gaussian(abs(texture_coords.x - 0.5), ray_width);
    float ray_inner = gaussian(abs(texture_coords.x - 0.5), ray_width * 10.0);

    float ray = (ray_outer + ray_inner) * clamp(fade, 0.0, 1.0);
    ray *= 1.0 + ball;

    float hue = fract(mix(hue - 0.25, hue, texture_coords.y * norm.y));
    ray *= fade_out_fraction;
    strike *= envelope(1.0 - fade_out_fraction, 0.05, 0.4);

    ray *= gaussian(abs(1.0 - texture_coords.y), 1.0 / 5.0);

    float light = smooth_max(strike * 3.0, ray, 0.5);

    return vec4(vec3(light), strike + ray) * vec4(lch_to_rgb(vec3(0.8, 1.0, clamp(hue, 0.0, 1.0))), 1.0);
}
