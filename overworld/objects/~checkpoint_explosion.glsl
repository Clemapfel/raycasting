...
vec4 effect(vec4 color, Image image, vec2 texture_coords, vec2 vertex_position) {
    ...
    float distortion_scale = 20 / camera_scale;
    float distortion_speed = 1 / 2.0;

    vec2 distortion = vec2(
    gradient_noise(vec3(to_uv(vertex_position) * distortion_scale, elapsed * distortion_speed)),
    gradient_noise(vec3(to_uv(vertex_position) * distortion_scale, elapsed * distortion_speed))
    ) * distortion_strength;

    uv += distortion;

    vec2 pxy = player_position / max(size.x, size.y);

    float f = fraction * camera_scale;

    float sigma = 180 * (player_radius / love_ScreenSize.y);
    float outer_fraction = mix(-0.05, 1, f);
    float inner_fraction = mix(-0.02, 1, f);

    float outer = gaussian(distance(pxy, uv) - outer_fraction, sigma);
    outer *= gaussian(f, 1 / camera_scale * 2 * 1.8);

    float inner = gaussian(distance(pxy, uv) - inner_fraction, sigma * 12);
    inner *= gaussian(f, 1 / camera_scale * 2 * 1.5);

    float ball = gaussian(distance(pxy, uv) / player_radius, 12 * player_radius) * (1 - fraction);
