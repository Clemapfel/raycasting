...
function ow.Tileset:instantiate(tileset_name)
    ...
    for tile in values(_get(config, "tiles")) do
        ...
        if tile.objectGroup ~= nil then
            local objects = ow.ObjectWrapper.parse_object_groups(tileset_name .. " Tile #" .. id, { _get(tile, "objectGroup") })
            to_push.objects = objects[1] -- first layer
    ...
    if space_usage < 0.7 then
        rt.warning("In ow.Tileset: texture atlas of tileset `", self._id .. "` only uses `",
            math.floor(space_usage * 1000) / 1000 * 100, "%` of allocated space"
        )
    end
