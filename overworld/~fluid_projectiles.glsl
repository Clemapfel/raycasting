...
vec4 effect(vec4 color, sampler2D image, vec2 texture_coordinates, vec2 frag_position) {
    vec2 pixel_size = vec2(1.0 / 1000.0);

    float tl = texture(image, texture_coordinates + vec2(-1.0, -1.0) * pixel_size).a;
    float tm = texture(image, texture_coordinates + vec2( 0.0, -1.0) * pixel_size).a;
    float tr = texture(image, texture_coordinates + vec2( 1.0, -1.0) * pixel_size).a;
    float ml = texture(image, texture_coordinates + vec2(-1.0, 0.0) * pixel_size).a;
    float mr = texture(image, texture_coordinates + vec2( 1.0, 0.0) * pixel_size).a;
    float bl = texture(image, texture_coordinates + vec2(-1.0, 1.0) * pixel_size).a;
    float bm = texture(image, texture_coordinates + vec2( 0.0, 1.0) * pixel_size).a;
    float br = texture(image, texture_coordinates + vec2( 1.0, 1.0) * pixel_size).a;
