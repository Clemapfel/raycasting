...
vec4 effect(vec4 color, sampler2D img, vec2 texture_coords, vec2 vertex_position) {
    ...
    if (distance(value, 1.0) < 0.001) return vec4(0.0, 0.0, 0.0, 1.0);
    ...
    vec2 noise_scale = vec2(3.0, 2.0) * 4.0;

    const float angle_width = 0.5 * 2.0 * PI;
    const float angle_direction = 0.0; // upwards
    ...
    for (int i = 0; i < n_octaves; ++i) {
        ...
        float angle = mix(angle_direction - angle_width / 2.0, angle_direction + angle_width / 2.0, (gradient_noise(vec3(seed.xy, elapsed)) + 1.0) / 2.0);
        vec2 offset = vec2(
        cos(angle),
        sin(angle)
        );
    ...
    float noise = 0.0;
    if (direction > 0.0) { // fade in: wipe upwards
        // For vertical wipe, aspect ratio is not needed
        noise = distance(pos.y, norm_coords.y);
        noise = 1.0 - clamp(norm_coords.y / 2.0 + noise, 0.0, 1.0);
    }
    else if (direction < 0.0) { // fade out: circle wipe
        // Use aspect-corrected coordinates for circular wipe
        float dist = distance(norm_coords, norm_center) * (value + 1.0 + distance(pos.y, norm_coords.y));
        noise = 1.0 - dist;
    }

    float opacity = 1.0 - smoothstep(value - eps, value + eps, noise);

    float border_eps = 0.07;
    float border_band = 1.0 - gaussian(
    ...
    return vec4(lch_to_rgb(vec3(0.8, 1.0, pos.x)) * border_band, color.a * opacity);
}
