...
vec4 effect(vec4 color, sampler2D tex, vec2 texture_coords, vec2 screen_coords) {
    ...
    float weight = 1.0 - 0.6 * distance(screen_coords.xy, love_ScreenSize.xy * 0.5) / (min(love_ScreenSize.x, love_ScreenSize.y) * 0.5);
    weight = 1.0 - weight * 10.0 * 0.5;
    ...
    screen_coords.xy += time_direction * elapsed * 250.0;

    vec3 ray_position = vec3(screen_coords * 0.01, 0.0);
    vec3 ray_direction = vec3(0.0, 0.0, 1);
    ...
    for (int i = 0; i < n_steps; ++i) {
        ...
        float density = density_multiplier * (1.0 + gradient_noise(sample_pos)) / 2.0;
        density *= weight;
    ...
    return vec4(lch_to_rgb(vec3(0.8, 1.0, hue)), accumulated_alpha * opacity);
}
