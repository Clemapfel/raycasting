...
const damping = 0.95;
const max_agitation = 2;
const agitation_duration = 1.5;
...
const vertex_r_offset = 4;
const vertex_g_offset = 5;
const vertex_b_offset = 6;
const vertex_a_offset = 7;

const mesh_vertex_stride = vertex_a_offset + 1;
const mesh_data_stride = 4 * mesh_vertex_stride;
...
export class Orb extends GLWidget  {
    ...
    protected override draw() {
        ...
        this.particle_mesh_shader.setUniform(DEFAULT_TEXTURE_UNIFORM_NAME, this.particle_mesh_texture);
    ...
    protected override async realize() {
        ...
        this.orb_radius = Math.min(size.x, size.y);
        this.orb_center_x = 0.5 * size.x;
        this.orb_center_y = 0.5 * size.y;

        this.particle_mesh_vertex_data = new Float32Array(n_particles * mesh_data_stride);
        ...
        for (let particle_i = 0; particle_i < n_particles; ++particle_i) {
            ...
            const index_data_i = particle_i * index_data_stride;
            ...
            const mass_t = mass_distribution(t);
            ...
            this.particle_mesh_vertex_data[index_data_i + 0] = vertex_base + 0;
            this.particle_mesh_vertex_data[index_data_i + 1] = vertex_base + 1;
            this.particle_mesh_vertex_data[index_data_i + 2] = vertex_base + 2;
            this.particle_mesh_vertex_data[index_data_i + 3] = vertex_base + 0;
            this.particle_mesh_vertex_data[index_data_i + 4] = vertex_base + 2;
            this.particle_mesh_vertex_data[index_data_i + 5] = vertex_base + 3;
        }

        this.particle_mesh = new Mesh(this.context, this.particle_mesh_vertex_data, this.particle_mesh_index_data);
    ...
    private update_mesh_data() {
        for (let particle_i = 0; particle_i < n_particles; ++particle_i) {
            const vertex_data_i = particle_i * particle_stride;
            const mesh_data_i = particle_i * mesh_data_stride;
            
            let px = this.particle_data[vertex_data_i + x_offset];
            let py = this.particle_data[vertex_data_i + y_offset];
            const vx = this.particle_data[vertex_data_i + velocity_x_offset];
            const vy = this.particle_data[vertex_data_i + velocity_y_offset];
            const r = this.particle_data[vertex_data_i + r_offset] * blend_strength;
            const g = this.particle_data[vertex_data_i + g_offset] * blend_strength;
            const b = this.particle_data[vertex_data_i + b_offset] * blend_strength;
            const a = this.particle_data[vertex_data_i + a_offset] * 1;
            const radius_cycle = this.particle_data[vertex_data_i + radius_cycle_offset]

            const elapsed = this.elapsed.asSeconds();
            let radius = this.particle_data[vertex_data_i + radius_offset] * texture_scale
            ...
            let offset = 0;
            data[mesh_data_i + offset + vertex_x_offset] = top_left_x;
            data[mesh_data_i + offset + vertex_y_offset] = top_left_y;
            data[mesh_data_i + offset + vertex_u_offset] = 0;
            data[mesh_data_i + offset + vertex_v_offset] = 0;
            data[mesh_data_i + offset + vertex_r_offset] = r;
            data[mesh_data_i + offset + vertex_g_offset] = g;
            data[mesh_data_i + offset + vertex_b_offset] = b;
            data[mesh_data_i + offset + vertex_a_offset] = a;

            offset += mesh_vertex_stride;
            data[mesh_data_i + offset + vertex_x_offset] = top_right_x;
            data[mesh_data_i + offset + vertex_y_offset] = top_right_y;
            data[mesh_data_i + offset + vertex_u_offset] = 1;
            data[mesh_data_i + offset + vertex_v_offset] = 0;
            data[mesh_data_i + offset + vertex_r_offset] = r;
            data[mesh_data_i + offset + vertex_g_offset] = g;
            data[mesh_data_i + offset + vertex_b_offset] = b;
            ...
            offset += mesh_vertex_stride;
            data[mesh_data_i + offset + vertex_x_offset] = bottom_right_x;
            data[mesh_data_i + offset + vertex_y_offset] = bottom_right_y;
            data[mesh_data_i + offset + vertex_u_offset] = 1;
            data[mesh_data_i + offset + vertex_v_offset] = 1;
            data[mesh_data_i + offset + vertex_r_offset] = r;
            data[mesh_data_i + offset + vertex_g_offset] = g;
            data[mesh_data_i + offset + vertex_b_offset] = b;
            data[mesh_data_i + offset + vertex_a_offset] = a;
            ...
            offset += mesh_vertex_stride;
            data[mesh_data_i + offset + vertex_x_offset] = bottom_left_x;
            data[mesh_data_i + offset + vertex_y_offset] = bottom_left_y;
            data[mesh_data_i + offset + vertex_u_offset] = 0;
            data[mesh_data_i + offset + vertex_v_offset] = 1;
            data[mesh_data_i + offset + vertex_r_offset] = r;
            data[mesh_data_i + offset + vertex_g_offset] = g;
    ...
    private step(delta : number) {
        ...
        const xy = new Vec2();
        const dxy = new Vec2();
        const orb_xy = new Vec2(this.orb_center_x, this.orb_center_x);
