...
uniform float saturation_fraction; // 0: original color 1: monochrome
uniform float vignette_fraction; // 0: no vignette, 1: screen fully blacked out
uniform float ripple_fraction; // 0: no ripple, 1 maximum ripple
uniform float elapsed; // seconds

vec4 effect(vec4 color, sampler2D img, vec2 texture_coords, vec2 frag_position) {

    if (isinf(elapsed)) // catch math.huge
    ...
    // ripple
    const float ripple_scale = 15;  // the higher, the more new lines appear
    const float ripple_speed = 2.5; // how fast new lines appear
    const float ripple_magnitude = 1.0 / 2.2; // how far the lines reach towards the center
