    ...
    do -- generate entries
        ...
        for id, path in pairs(id_to_path) do
            self._id_to_entry[id] = {
                ...
                was_processed = false,
                equalizer_entry_rms = 0, -- root mean square
                equalizer_volume = 1
            }
        end
    end
...
do
    ...
    function rt.SoundManager:_import_audio(sound_path)
        ...
        local attack = rt.settings.sound_manager.default_attack
        local release = rt.settings.sound_manager.default_release
        ...
        for i = 0, sample_count - 1 do
            ...
            -- apply envelope
            local enveloped_sample = sample * envelope:at(i / sample_rate)
...
function rt.SoundManager:_play(handler_id, id, config)
    ...
    if config.attack == nil then config.attack = 0 end
    if config.release == nil then config.release = 0 end

    for key in keys(config) do
        if _config_valid_keys[key] ~= true then
            rt.critical("In rt.SoundManager.player: unrecognized option `", key,  "`, it will be ignored")
        end
    end
    ...
    entry.envelope = rt.Envelope(
        config.attack,
        entry.duration - math.min(config.attack + config.release, entry.duration),
        config.release,
        rt.EnvelopeCurve.LINEAR,
        rt.EnvelopeCurve.LINEAR
    )
    ...
    source:setVolume(entry.equalizer_volume * self._volume * entry.envelope:at(0))
    ...
    for entry in keys(self._active_entries) do
        for handler_id, source in pairs(entry.handler_id_to_active_sources) do
            if source ~= nil then
                ...
                source:setVolume(motion_value * entry.equalizer_volume * self._volume * entry.envelope:get_value())
            end
        end
    end
end
