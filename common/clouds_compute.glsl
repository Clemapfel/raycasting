...
#error "EXPORT_TEXTURE_FORMAT undefined"
#endif
...
void computemain() {
    ...
    for (int export_texture_i = 0; export_texture_i < n_export_textures; ++export_texture_i) {
        vec3 ray_position = vec3(
        uv.xy,
        float(export_texture_i) / float(n_export_textures) // slice offset
        );
        ...
        for (int density_step_i = 0; density_step_i < n_density_steps; ++density_step_i) {
            ...
            if (sampled_density > 0.01) {
                ...
                // stop if transmittance is too low (opaque cloud)
        ...
        // final alpha is inverse of transmittance (1 - transmittance)
        color.a = 1.0 - transmittance;

        // write final color to export texture slice
        imageStore(export_textures[export_texture_i], gid.xy, color);
    }
