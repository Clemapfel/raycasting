...
function rt.TimedAnimationSequence:instantiate(...)
    for i = 1, select('#', ...) do
        meta.assert_argument_type(select(i, ...), rt.TimedAnimation, i)
    end
...
rt.AnimationChain = function(...)
    ...
    while i < n do
        ...
        meta.assert_argument_type(duration, mt.Number, i + 0)

        local min = select(i + 1, ...)
        meta.assert_argument_type(min, mt.Number, i + 1)

        local max = select(i + 2, ...)
        meta.assert_argument_type(max, mt.Number, i + 2)

        local f = select(i + 3, ...)
        meta.assert_argument_type(f, mt.Function, i + 3)
