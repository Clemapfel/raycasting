...
function rt.PlayerBody:initialize(positions)
    ...
    local n_segments = 8
    local n_ropes = n_rings * n_ropes_per_ring

    if #self._ropes == 0 then
        for ring = 1, n_rings do
            ...
            for i = 1, current_n_ropes do
                ...
                for j = 1, n_segments do
                    local delta = (j - 1) / n_segments * rope_length
                    ...
                    table.insert(rope.bubble_distances, 1 / n_segments)
                end
...
local function _solve_bending_constraint(a_x, a_y, b_x, b_y, c_x, c_y, rest_length, stiffness)
    ...
    local target_x, target_y = math.normalize(ab_x + bc_x, ab_y + bc_y)
...
function rt.PlayerBody:update(delta)
    ...
    while self._elapsed > _step do
        ...
        for rope in values(self._ropes) do
            ...
            local n_bending_iterations_done = 1
            ...
            while (apply_axis and n_axis_iterations_done < _n_axis_iterations) or
                (apply_bending and n_bending_iterations_done < _n_bending_iterations) or
                n_distance_iterations_done <_n_distance_iterations or
                n_velocity_iterations_done < _n_velocity_iterations do
                -- velocity
                if n_velocity_iterations_done < _n_velocity_iterations then
                ...
                -- axis
                if apply_axis and n_axis_iterations_done < _n_axis_iterations then
                ...
                -- bending
                if apply_bending and n_bending_iterations_done < _n_bending_iterations then
                    local distance_i = 1
                ...
                -- distance
                if n_distance_iterations_done < _n_distance_iterations then
                    local distance_i = 1
                    for i = 1, #positions - 2, 2 do
                        local node_1_xi, node_1_yi, node_2_xi, node_2_yi = i, i+1, i+2, i+3
                        local node_1_x, node_1_y = positions[node_1_xi], positions[node_1_yi]
...
function rt.PlayerBody:draw_core()
    ...
    local outline_width = 2 / self._player:get_radius()
    ...
    -- draw outline
    local darken = 0.5 -- TODO: settings
