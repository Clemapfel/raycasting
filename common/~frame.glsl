...
vec4 effect(vec4 color, sampler2D img, vec2 texture_coords, vec2 screen_coords) {
    ...
    uv.y = min(uv.y + 0.5 * (gradient_noise(vec3(vec2(4.0 * (screen_coords.x / love_ScreenSize.x)), elapsed)) + 1.0) / 2.0, 1.0);
    return vec4(vec3(mix(color - 0.1, color, 1.0 - clamp(gaussian(1.0 - uv.y, 1.5), 0.0, 1.0))), 1.0);
